...
Il codice contiene **alcuni anti-pattern intenzionali** inseriti come "esca": sono esattamente ciò che i tuoi subagent reviewer (`security-reviewer`, `code-reviewer-banking-domain`, `performance-reviewer`) e la tua Code Review Suite devono **individuare da soli** durante il bootcamp.

Il punto dell'esercizio è che sia il *subagent* a trovarli — quindi qui **non** li elenchiamo. Riguardano le aree classiche del backend banking: gestione dei secret, idempotency delle operazioni che muovono denaro, e performance dell'accesso ai dati. Lancia i tuoi reviewer e lascia che li scoprano.

> Il dettaglio completo (file, riga, severity, fix enterprise) è nella versione docente del progetto (`solutions/liparibank-spring-target/README.md` nei materiali docente) — non in questa copia studente.
...
## Open Code Assets
- Creati 3 agent specifici per verifiche in base allo scope con funzioni Only-Read, riportando le anomalie in ordine di gravità:
  - **"code-reviewer-banking-domain"** effettuare una review con scope AML o il Dominio Banking.
  - **"code-reviewer-amount-validation"** effettuare una review con scope su gli importi.
  - **"code-reviewer-external-api"** effettuare una review con scope verifica sulle api esterne.
- Creato skill **"compliance-aml-check"** con lo scopo di verificare che, in fase di cambiamento di codice, le modifiche rimangono congrue ai requisiti aml.
- Creato commands **"code-review"** per far partire il **"code-reviewer-banking-domain"**.
- Creato **"Agents"** nella directory principale per elencare vincoli e comportamenti generali per gli agents:
  - Avvio automatico di un code reviewer in base allo scope con vincoli in caso di ambiguita o assenza di scope.
  - Avvio automatico del salvataggio di un report con nomenclatura alla fine dell'esecuzione di un agents di review.